...
export default function SellPage() {
  ...
  // เพิ่มสินค้าเข้าตะกร้าแบบคลิกเดียวจากปุ่มลัด
  ...
  async function handleConfirmSale() {
    ...
    // ตรวจสอบ stock ล่าสุดอีกครั้งก่อนบันทึกจริง
    ...
    // 2. อัปเดต stock ของสินค้าแต่ละรายการ + ส่งแจ้งเตือน Telegram
    for (const item of cart) {
      const fresh = freshProducts.find((p) => p.id === item.productId);
      ...
      // ---- แจ้งเตือนเข้า Telegram หลังตัดสต็อกสำเร็จ ----
                    ...
                    <tr key={item.productId}>
                      ...
                      <td>
                        <input
                          ...
                          max={item.stock}
                          value={item.quantity}
                          onChange={(e) => handleChangeCartQuantity(item.productId, e.target.value)}
                          style={{ width: "70px" }}
                        />
                      </td>
